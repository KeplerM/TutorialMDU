...
#
    ...
    for i in range(0,1,1):
        Q11.append((E11**2/(E11-Nu12**2*E22)))
    ...
    for j in range (0,PlyN,1):
        Q11_S.append((Q11[i]*np.cos(Lam[j]*np.pi/180.0)**4)+(Q22[i]*np.sin(Lam[j]*np.pi/180.0)**4)+((Q12[i]+2*Q66[i])*np.cos(Lam[j]*np.pi/180.0)**2*np.sin(Lam[j]*np.pi/180.0)**2))
